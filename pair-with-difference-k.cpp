Link: https://www.hackerrank.com/contests/smart-interviews/challenges/si-pair-with-difference-k
/*
...
int main() {
    ...
    while(t--){
        ...
        string flag="false";
        for(int i=0;i<n-1;i++){
           if(binary_search(arr+i+1,arr+n,arr[i]-k) || binary_search(arr+i+1,arr+n,arr[i]+k)) {
               flag="true";
                break;
           }
        }
